...
internal static class CraftingListHelpers
{
    ...
    public static void AddRecipeIngredientsToList(Recipe? recipe, ref Dictionary<uint, int> ingredientList, bool addSublist = true, CraftingList selectedList = null)
    {
        ...
        try
        {
            ...
            {
                var quantity = (selectedList != null)
                        ? ing.AmountIngredient * selectedList.Items.Count(x => x == recipe.RowId)
                        : ing.AmountIngredient;
                if (ingredientList.ContainsKey((uint)ing.ItemIngredient))
                {
                    ingredientList[(uint)ing.ItemIngredient] += quantity;
                }
                else
                {
                    ingredientList.TryAdd((uint)ing.ItemIngredient, quantity);
                }
                ...
                if (addSublist && HasIngredientRecipe((uint)ing.ItemIngredient))
                {
    ...
    public static bool HasIngredientRecipe(uint ingredient)
    {
        return LuminaSheets.RecipeSheet.Values.Any(x => x.ItemResult.Value.RowId == ingredient);
    }
